...
function manageBanners(bannerWrapper: HTMLElement | null, path: string): void {
    ...
    const hasSpecialBannerPath = Object.values(window.bannerType).some(bannerPath => path.includes(bannerPath));

    // Is it a special path that has the banner expected by the path?
    allBanners.forEach(banner => {
        const existingBanner = banner.getAttribute('banner-type');
        if (existingBanner && path.includes(window.bannerType[existingBanner])) { hasExpectedBanner = true };
    })

    allBanners.forEach(banner => {
        const currentBannerType: string = banner.getAttribute('banner-type') || '';
        if (!currentBannerType) { return }
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const nav = document.querySelector('[pp-type="nav-wrapper"]') as HTMLElement | null;
    const bannerWrapper = nav?.querySelector('[pp-type="infobanner-component"]') as HTMLElement | null;
