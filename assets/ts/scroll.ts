export function scrollToSection(id: string, offset: number = 0, selectorType: 'id' | 'any' = 'id'): void {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (location.pathname !== '/') {
    const globalScrollLinks = document.querySelectorAll<HTMLAnchorElement>('a[data-global-scroll="true"]');
    const globalFiltered = Array.from(globalScrollLinks).filter(link => link.href.includes(location.pathname));
  ...
  const allScrollLinks = document.querySelectorAll<HTMLAnchorElement>('a[href^="#"], [scroll-to]');
