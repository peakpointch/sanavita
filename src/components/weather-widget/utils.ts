...
export function getDaysFromForecast(
  ...
  const dailyGroups = forecast.list.reduce<Record<string, DayForecast>>((acc, item) => {
    // Parse date
    ...
    if (!acc || !acc[dateStr]) {
      ...
      acc[dateStr].minTemp = Math.min(acc[dateStr].minTemp, item.main.temp_min);
      acc[dateStr].maxTemp = Math.max(acc[dateStr].maxTemp, item.main.temp_max);
    }
