...
export default class SaveOptions<ActionKey extends string = string> {
  ...
  constructor(component: HTMLElement, settings: Partial<SaveOptionsSettings> = {}) {
    this.component = component;
    this.settings = deepMerge(SaveOptions.defaultSettings, settings);
    this.instance = this.settings.id || component.getAttribute(SaveOptions.attr.id);
    ...
    dropdownOptions.forEach((optionEl) => {
      ...
      const label = optionEl.getAttribute(SaveOptions.attr.label) || optionEl.textContent?.trim() || '';
      if (!key) return;
  ...
  private static attributeSelector = createAttribute<SaveOptionsElement>(SaveOptions.attr.element);

  /**
   * Static selector
   */
  public static selector(element: SaveOptionsElement, instance?: string): string {
    const base = SaveOptions.attributeSelector(element);
    return instance
      ? `${base}[${SaveOptions.attr.id}="${instance}"]`
      : base;
  }
  ...
  public static select<T extends Element = HTMLElement>(element: SaveOptionsElement, instance?: string): T {
    return document.querySelector<T>(SaveOptions.selector(element, instance));
  }

  public static selectAll<T extends Element = HTMLElement>(element: SaveOptionsElement, instance?: string): NodeListOf<T> {
    return document.querySelectorAll<T>(SaveOptions.selector(element, instance));
  }

  public select<T extends Element = HTMLElement>(element: SaveOptionsElement, local: boolean = true): T {
    return local
  ...
  public selectAll<T extends Element = HTMLElement>(element: SaveOptionsElement, local: boolean = true): NodeListOf<T> {
    return local
      ? this.component.querySelectorAll<T>(SaveOptions.selector(element))
      : document.querySelectorAll<T>(SaveOptions.selector(element, this.instance))
  }
