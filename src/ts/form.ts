() => {
  ...
  function initForm(component: HTMLElement | null) {
    ...
    const form = component.querySelector(
      FORM_SELECTOR
    ) as HTMLFormElement | null; // Has to be a HTMLFormElement because its selector is the form tagname
    if (!form) {
      console.error(
        `The selected form component does not contain a HTMLFormElement. Perhaps you added ${COMPONENT_SELECTOR} to the form element itself rather than its parent element?\n\nForm Component:`,
        component
      );
      return false;
    }
  ...
  async function handleSubmit(component: HTMLElement, form: HTMLFormElement) {
    ...
    // Form elements
    const allInputs: NodeListOf<FormElement> =
      form.querySelectorAll(INPUT_SELECTOR);
    const successElement: HTMLElement | null =
      component.querySelector(SUCCESS_SELECTOR);
    const errorElement: HTMLElement | null =
      component.querySelector(ERROR_SELECTOR);
    const submitButton: HTMLInputElement =
      component.querySelector(SUBMIT_SELECTOR)!; // Not null because this form can't be submitted without a submit button
